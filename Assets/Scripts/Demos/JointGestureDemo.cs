...
public class JointGestureDemo : SingleAgentInteraction {
    ...
    void Update() {
        ...
        if (demoSurface != GlobalHelper.GetMostImmediateParentVoxeme(demoSurface)) {
        ...
        if (leftRegion == null) {
            leftRegion = new Region(new Vector3(GlobalHelper.GetObjectWorldSize(demoSurface).center.x,
                    GlobalHelper.GetObjectWorldSize(demoSurface).max.y + Constants.EPSILON,
                    GlobalHelper.GetObjectWorldSize(demoSurface).min.z + Constants.EPSILON),
                new Vector3(GlobalHelper.GetObjectWorldSize(demoSurface).max.x - Constants.EPSILON,
                    GlobalHelper.GetObjectWorldSize(demoSurface).max.y + Constants.EPSILON,
                    GlobalHelper.GetObjectWorldSize(demoSurface).max.z - Constants.EPSILON));
        ...
        if (rightRegion == null) {
            rightRegion = new Region(new Vector3(GlobalHelper.GetObjectWorldSize(demoSurface).min.x + Constants.EPSILON,
                    GlobalHelper.GetObjectWorldSize(demoSurface).max.y + Constants.EPSILON,
                    GlobalHelper.GetObjectWorldSize(demoSurface).min.z + Constants.EPSILON),
                new Vector3(GlobalHelper.GetObjectWorldSize(demoSurface).center.x,
                    GlobalHelper.GetObjectWorldSize(demoSurface).max.y + Constants.EPSILON,
                    GlobalHelper.GetObjectWorldSize(demoSurface).max.z - Constants.EPSILON));
        ...
        if (frontRegion == null) {
            frontRegion = new Region(new Vector3(GlobalHelper.GetObjectWorldSize(demoSurface).min.x + Constants.EPSILON,
                    GlobalHelper.GetObjectWorldSize(demoSurface).max.y + Constants.EPSILON,
                    GlobalHelper.GetObjectWorldSize(demoSurface).min.z + Constants.EPSILON),
                new Vector3(GlobalHelper.GetObjectWorldSize(demoSurface).max.x + Constants.EPSILON,
                    GlobalHelper.GetObjectWorldSize(demoSurface).max.y + Constants.EPSILON,
                    GlobalHelper.GetObjectWorldSize(demoSurface).center.z));
        ...
        if (backRegion == null) {
            backRegion = new Region(new Vector3(GlobalHelper.GetObjectWorldSize(demoSurface).min.x + Constants.EPSILON,
                    GlobalHelper.GetObjectWorldSize(demoSurface).max.y + Constants.EPSILON,
                    GlobalHelper.GetObjectWorldSize(demoSurface).center.z),
                new Vector3(GlobalHelper.GetObjectWorldSize(demoSurface).max.x + Constants.EPSILON,
                    GlobalHelper.GetObjectWorldSize(demoSurface).max.y + Constants.EPSILON,
        ...
        if (disableHighlight) {
            ...
            regionHighlight.GetComponent<Renderer>().material.color = new Color(1.0f, 1.0f, 1.0f,
                (1.0f / ((regionHighlight.transform.position -
                          new Vector3(demoSurface.transform.position.x, GlobalHelper.GetObjectWorldSize(demoSurface).max.y,
...
//          }
        ...
        if (allowDeixisByClick) {
            if (Input.GetMouseButtonDown(0)) {
                ...
                if (hit.collider != null) {
                    if (availableObjs.Contains(GlobalHelper.GetMostImmediateParentVoxeme(hit.collider.gameObject))) {
                        ...
                        if (dianaMemory != null && dianaMemory.enabled) {
                            ...
                            if (dianaMemory.IsKnown(GlobalHelper.GetMostImmediateParentVoxeme(hit.collider.gameObject)
                                .GetComponent<Voxeme>())) {
                                //Deixis (GlobalHelper.GetMostImmediateParentVoxeme (hit.collider.gameObject));
                                OnObjectSelected(this,
                                    new SelectionEventArgs(
                                        GlobalHelper.GetMostImmediateParentVoxeme(hit.collider.gameObject)));
                            }
                        }
                    }
                    else if (GlobalHelper.GetMostImmediateParentVoxeme(hit.collider.gameObject) == demoSurface) {
                        if (!epistemicModel.engaged) {
                            epistemicModel.engaged = true;
                        }
        ...
        if (logFullState) {
            foreach (Voxeme voxeme in objSelector.allVoxemes) {
                if ((voxeme.gameObject.activeInHierarchy) &&
                    (!objSelector.disabledObjects.Contains(GlobalHelper.GetMostImmediateParentVoxeme(voxeme.gameObject)))) {
                    logger.OnLogEvent(this, new LoggerArgs(
                        string.Format("{0}\t{1}\t{2}",
                            logIndex.ToString(),
                            "", string.Format("{0}:{1},{2}", voxeme.gameObject.name,
                                GlobalHelper.VectorToParsable(voxeme.gameObject.transform.position),
                                GlobalHelper.VectorToParsable(voxeme.gameObject.transform.eulerAngles)))));
                }
            }
        }
    ...
    public void Wait(object[] content) {
        // type check
        if (!GlobalHelper.CheckAllObjectsOfType(content, typeof(string))) {
            return;
        }
    ...
    public void TrackPointing(object[] content) {
        ...
        switch (content.Length) {
            ...
            case 1:
                ...
                //      Vector3 origin = new Vector3 (vector [0], GlobalHelper.GetObjectWorldSize (demoSurface).max.y, vector [1]);
            ...
            case 1:
                ...
                if (message.Contains("there")) {
                    if ((regionHighlight.GetComponent<Renderer>().material == activeHighlightMaterial) &&
                        (regionHighlight.transform.position.y > 0.0f)) {
                        if (!GlobalHelper.RegionsEqual(interactionLogic.IndicatedRegion, new Region())) {
            ...
            case 1:
                ...
                if (interactionLogic.RemoveInputSymbolType(message, interactionLogic.GetInputSymbolType(message))
                    ...
                    Region testRegion = new Region(
                        new Vector3(GlobalHelper.GetObjectWorldSize(demoSurface).min.x, regionHighlight.transform.position.y,
                            GlobalHelper.GetObjectWorldSize(demoSurface).min.z),
                        new Vector3(GlobalHelper.GetObjectWorldSize(demoSurface).max.x, regionHighlight.transform.position.y,
                            GlobalHelper.GetObjectWorldSize(demoSurface).max.z));
    ...
    public void DisambiguateObject(object[] content) {
        ...
        if (allOptionsSameType || anyGlobalSameType) {
            for (int i = 0; i < objVoxemes.Count; i++) {
                List<object> newAttrs = GlobalHelper.DiffLists(
...
//      Debug.Log (interactionLogic.IndicatedObj.name);
        ...
        if (duplicateNominalAttr) {
            ...
            if (interactionLogic.ActionOptions.Count == 0) {
                ...
                object theme = eventManager.ExtractObjects(GlobalHelper.GetTopPredicate(interactionLogic.ActionOptions[0]),
                    (String) GlobalHelper.ParsePredicate(interactionLogic.ActionOptions[0])[
                        GlobalHelper.GetTopPredicate(interactionLogic.ActionOptions[0])])[0];
            ...
            case 1:
                ...
                if (message == "big") {
                    obj = interactionLogic.ObjectOptions.OrderByDescending(o =>
                        GlobalHelper.GetObjectWorldSize(o).size.x *
                        GlobalHelper.GetObjectWorldSize(o).size.y *
                        GlobalHelper.GetObjectWorldSize(o).size.z).ToList()[0];
                }
        ...
        switch (content.Length) {
            ...
            case 1:
                ...
                if (((List<string>) symbolContent.ActionOptions).Count > 0) {
                    ...
                    obj = eventManager.ExtractObjects(GlobalHelper.GetTopPredicate(instructionCmd),
                            (String) GlobalHelper.ParsePredicate(instructionCmd)[GlobalHelper.GetTopPredicate(instructionCmd)])[0]
                        as
                        GameObject;
            ...
            interactionLogic.GenerateStackSymbol(null, null, null, null,
                new List<string>(new string[] {
                    string.Format("put({0},{1})", interactionLogic.IndicatedObj.name,
                        GlobalHelper.VectorToParsable(interactionLogic.IndicatedRegion.center))
                }),
                null)));
    }
    ...
    public void StartGrab(object[] content) {
        ...
        if (interactionLogic.ActionOptions.Count > 0) {
            ...
            objs = eventManager.ExtractObjects(GlobalHelper.GetTopPredicate(actionCmd),
                (String)GlobalHelper.ParsePredicate(actionCmd)[GlobalHelper.GetTopPredicate(actionCmd)]);
        ...
        //GlobalHelper.GetTopPredicate(interactionLogic.ActionOptions[0]),
        //(String)GlobalHelper.ParsePredicate(interactionLogic.ActionOptions[0])[
        //GlobalHelper.GetTopPredicate(interactionLogic.ActionOptions[0])])[0]);
            ...
            if (interactionLogic.IndicatedObj.GetComponent<Voxeme>().graspConvention == null) {
                if (grabPoses.Count > 1) {
                    ...
                    List<GameObject> excludeChildren = interactionLogic.IndicatedObj.GetComponentsInChildren<Renderer>()
                        .Where(
                            o => (GlobalHelper.GetMostImmediateParentVoxeme(o.gameObject) != interactionLogic.IndicatedObj))
                        .Select(v => v.gameObject).ToList();
                    foreach (GameObject child in excludeChildren) {
                        //Debug.Log(child);
                        //Debug.Log(GlobalHelper.VectorToParsable(GlobalHelper.GetObjectWorldSize(child.gameObject).size));
                        //Debug.Log(GlobalHelper.VectorToParsable(
                        //GlobalHelper.GetObjectWorldSize(interactionLogic.IndicatedObj, excludeChildren).size));
                        if (!GlobalHelper.FitsIn(GlobalHelper.GetObjectWorldSize(child.gameObject),
                            GlobalHelper.GetObjectWorldSize(interactionLogic.IndicatedObj, excludeChildren), true)) {
                            //Debug.Log(child);
...
//                  options.ToArray().Reverse().ToList().GetRange(0,s+1), new List<string>())).ToList()));
    ...
    public void Servo(object[] content) {
        ...
        switch (content.Length) {
            ...
            case 1:
                ...
                Debug.Log(GlobalHelper.VectorToParsable(obj.transform.position));
                Debug.Log(GlobalHelper.VectorToParsable(destCoord));

                string eventStr = string.Format("slidep({0},{1})", obj.name, GlobalHelper.VectorToParsable(destCoord));
                ...
                if (interactionLogic.LearnableInstructions.ContainsKey(
                    ...
                    string learnedInstructionString = string.Format("{0} the {1}",
                        ...
                        ((GameObject) eventManager.ExtractObjects(
                            GlobalHelper.GetTopPredicate(interactionLogic.ActionOptions[0]),
                            (String) GlobalHelper.ParsePredicate(interactionLogic.ActionOptions[0])[
                                GlobalHelper.GetTopPredicate(interactionLogic.ActionOptions[0])])[0]).name);
        ...
        if (interactionLogic.GraspedObj != null) {
            ...
            if ((!Physics.Raycast(new Ray(interactionLogic.GraspedObj.transform.position, Vector3.down),
                ...
                PromptEvent(string.Format("put({0},{1})",
                    ...
                    GlobalHelper.VectorToParsable(new Vector3(interactionLogic.GraspedObj.transform.position.x,
                        GlobalHelper.GetObjectWorldSize(demoSurface).max.y +
                        GlobalHelper.GetObjectWorldSize(interactionLogic.GraspedObj).extents.y,
                        interactionLogic.GraspedObj.transform.position.z))));
            }
            else {
                PromptEvent(string.Format("put({0},on({1}))",
                    interactionLogic.GraspedObj.name,
                    GlobalHelper.GetMostImmediateParentVoxeme(hitInfo.collider.gameObject).name));
            }
    ...
    public void CleanUp(object[] content) {
        if (interactionLogic.GraspedObj != null) {
            if (!RCC8.EC(GlobalHelper.GetObjectWorldSize(interactionLogic.GraspedObj),
                GlobalHelper.GetObjectWorldSize(interactionLogic.GraspedObj.GetComponent<Voxeme>().supportingSurface))) {
                RaycastHit hitInfo;
                ...
                if ((!Physics.Raycast(new Ray(interactionLogic.GraspedObj.transform.position, Vector3.down),
                    ...
                    PromptEvent(string.Format("put({0},{1})",
                        interactionLogic.GraspedObj.name,
                        GlobalHelper.VectorToParsable(new Vector3(interactionLogic.GraspedObj.transform.position.x,
                            GlobalHelper.GetObjectWorldSize(GlobalHelper.GetMostImmediateParentVoxeme(hitInfo.collider.gameObject))
                                .max.y,
                            interactionLogic.GraspedObj.transform.position.z))));
                }
            }
    ...
    Vector3 MoveHighlight(Vector3 highlightCenter, float variance = 0.0f) {
        ...
        if ((regionHighlight.transform.position.x + vectorConeRadius < GlobalHelper.GetObjectWorldSize(demoSurface).min.x) ||
            (regionHighlight.transform.position.x - vectorConeRadius > GlobalHelper.GetObjectWorldSize(demoSurface).max.x) ||
            (regionHighlight.transform.position.z + vectorConeRadius < GlobalHelper.GetObjectWorldSize(demoSurface).min.z) ||
            (regionHighlight.transform.position.z - vectorConeRadius > GlobalHelper.GetObjectWorldSize(demoSurface).max.z)) {
    ...
    List<GameObject> FindObjectsInRegion(Region region) {
        ...
        foreach (GameObject obj in availableObjs) {
            ...
            if ((obj.activeInHierarchy) || (objSelector.disabledObjects.Contains(obj))) {
                Vector3 point = GlobalHelper.GetObjectWorldSize(obj).ClosestPoint(highlightCenter);
            ...
            if (option is GameObject) {
                ...
                if (certainty == CertaintyMode.Act) {
                    if (!moveOptions.Contains(string.Format("put({0},{1})", theme.name,
                        GlobalHelper.VectorToParsable(target)))) {
                        ...
                        if (region.Contains(target)) {
                            if (!confirmationTexts.ContainsKey(string.Format("put({0},{1})", theme.name,
                                ...
                                confirmationTexts.Add(
                                    string.Format("put({0},{1})", theme.name, GlobalHelper.VectorToParsable(target)),
            ...
            if (option is GameObject) {
                ...
                if (theme != obj) {
                    if (FitsTouching(theme, obj, directionPreds[relativeDir[dir]]) &&
                        (GlobalHelper.GetObjectWorldSize(theme).min.y >=
                         GlobalHelper.GetObjectWorldSize(obj).min.y - Constants.EPSILON)) {
                        ...
                        else if (GlobalHelper.GetObjectWorldSize(theme).min.y < GlobalHelper.GetObjectWorldSize(obj).min.y) {
                ...
                if (certainty == CertaintyMode.Act) {
                    if (!pushOptions.Contains(string.Format("slide({0},{1})", theme.name,
                        ...
                        foreach (Region region in orthogonalRegions) {
                            if (region.Contains(target)) {
                                confirmationTexts.Add(
                                    string.Format("slide({0},{1})", theme.name, GlobalHelper.VectorToParsable(target)),
    ...
    List<object> FindPlacementOptions(GameObject theme, string dir) {
                    ...
                    if (block.activeInHierarchy) {
                        if (block != theme) {
                            ...
                            if ((bool) (Type.GetType("QSR").GetMethod(qsr).Invoke(null, new object[] {
                                    GlobalHelper.GetObjectWorldSize(block),
                                    themeBounds
        ...
        foreach (Region region in orthogonalRegions) {
            if (region.Contains(theme)) {
                // stay in this region
                target = GlobalHelper.FindClearRegion(demoSurface, new Region[] {thisRegion, region}, theme).center +
                         theme.transform.position - (GlobalHelper.GetObjectWorldSize(theme).center);
        ...
        if (!logActionsOnly) {
            logger.OnLogEvent(this, new LoggerArgs(
                string.Format("{0}\t{1}\t{2}",
                    (++logIndex).ToString(),
                    "AG",
                    string.Format("look_at({0})", GlobalHelper.VectorToParsable(point)))));
        }
    }
...
//              if (GlobalHelper.GetMostImmediateParentVoxeme (hit.collider.gameObject) == demoSurface) {
...
//              zCoord);
    ...
    bool SurfaceClear(GameObject block) {
        ...
        List<GameObject> excludeChildren = block.GetComponentsInChildren<Renderer>().Where(
            o => (GlobalHelper.GetMostImmediateParentVoxeme(o.gameObject) != block) ||
        ...
        foreach (GameObject otherBlock in availableObjs) {
            excludeChildren = otherBlock.GetComponentsInChildren<Renderer>().Where(
                o => (GlobalHelper.GetMostImmediateParentVoxeme(o.gameObject) != otherBlock) ||
                     (o.gameObject.layer == LayerMask.NameToLayer("blocks-known"))).Select(o => o.gameObject).ToList();
            foreach (GameObject go in excludeChildren) {
                Debug.Log(go);
            }

            Bounds otherBounds = GlobalHelper.GetObjectWorldSize(otherBlock, excludeChildren);
            Debug.Log(otherBlock);
            Debug.Log(otherBounds);
            Debug.Log(GlobalHelper.GetObjectWorldSize(otherBlock).min.y);
            Debug.Log(GlobalHelper.GetObjectWorldSize(otherBlock, excludeChildren).min.y);
...
//              (!QSR.Right (otherBounds, blockBounds)) && (RCC8.EC (otherBounds, blockBounds))) {
    ...
    public List<GameObject> GetGrabPoses(GameObject obj, GameObject hand) {
        ...
        poseList = poses.ToList().Where(
            p => GlobalHelper.GetMostImmediateParentVoxeme(p.gameObject) == obj).ToList().Where(
        ...
        if (!logActionsOnly) {
            logger.OnLogEvent(this, new LoggerArgs(
                string.Format("{0}\t{1}\t{2}",
                    (++logIndex).ToString(),
                    "AG",
                    string.Format("reach({0})", GlobalHelper.VectorToParsable(coord)))));
        }
    ...
    public void ReachFor(GameObject obj) {
        ...
        if (leftRegion.Contains(new Vector3(obj.transform.position.x,
            ...
            if (ikControl.rightHandObj.gameObject.GetComponent<TransformTarget>().enabled) {
                ...
                ikControl.rightHandObj.position = new Vector3(GlobalHelper.GetObjectWorldSize(obj).center.x,
                                                      GlobalHelper.GetObjectWorldSize(obj).max.y,
                                                      GlobalHelper.GetObjectWorldSize(obj).center.z) + offset;
            }
            ...
            if (ikControl.leftHandObj.gameObject.GetComponent<TransformTarget>().enabled) {
                ...
                ikControl.leftHandObj.position = new Vector3(GlobalHelper.GetObjectWorldSize(obj).center.x,
                                                     GlobalHelper.GetObjectWorldSize(obj).max.y,
                                                     GlobalHelper.GetObjectWorldSize(obj).center.z) + offset;
            }
        ...
        if (!logActionsOnly) {
            logger.OnLogEvent(this, new LoggerArgs(
                string.Format("{0}\t{1}\t{2}",
                    (++logIndex).ToString(),
                    "AG",
                    string.Format("point({0},{1})", hand.name, GlobalHelper.VectorToParsable(point)))));
        }
    }
        ...
        if (!leftGrasping) {
            if (leftTargetStored == new Vector3(float.MaxValue, float.MaxValue, float.MaxValue)) {
                ...
                Debug.Log(string.Format("Storing pose {0} {1} {2}",
                    GlobalHelper.VectorToParsable(ikControl.leftHandObj.transform.position),
                    GlobalHelper.VectorToParsable(ikControl.rightHandObj.transform.position),
                    GlobalHelper.VectorToParsable(ikControl.lookObj.transform.position)));
            }
        }
        else {
            leftTargetStored = new Vector3(float.MaxValue, float.MaxValue, float.MaxValue);
    ...
    public void ReturnToPose() {
        Debug.Log(string.Format("Returning to pose {0} {1} {2}",
            GlobalHelper.VectorToParsable(leftTargetStored),
            GlobalHelper.VectorToParsable(rightTargetStored),
            GlobalHelper.VectorToParsable(headTargetStored)));
...
//                      Debug.Log (string.Format("{0} certainty: {1}",concept.Name.ToLower (),concept.Certainty));
            ...
            if (!interactionSystem.IsPaused(FullBodyBipedEffector.LeftHand) &&
                ...
                if ((interactionLogic != null) && (interactionLogic.isActiveAndEnabled)) {
                    if ((interactionLogic.ActionOptions.Count > 0) &&
                        ...
                        string pred =
                            GlobalHelper.GetTopPredicate(
                                interactionLogic.ActionOptions[interactionLogic.ActionOptions.Count - 1]);
                        graspedObject = eventManager.ExtractObjects(pred,
                                (String) GlobalHelper.ParsePredicate(
                                    interactionLogic.
    ...
    void CompletedEventSequence(object sender, EventArgs e) {
        if (e != null) {
            ...
            MethodInfo method = predicates.GetType().GetMethod(GlobalHelper.GetTopPredicate(eventStr).ToUpper());
